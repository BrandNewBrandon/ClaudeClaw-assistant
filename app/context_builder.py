...
class ContextBuilder:
    ...
    def build_prompt(
        ...
        sections.append(
            ...
            "## Spawning Sub-Agents\n"
            "You can create new sibling agents on the user's request. Tools:\n"
            "- `create_agent(name, persona, display_name?, description?)` — scaffolds a new agent "
            "from the template at agents/_template/. Name must be lowercase letters/digits/dashes, "
            "1-32 chars.\n"
            "- `bind_channel(agent, channel, token?, app_token?, chat_identifier?)` — binds a "
            "communication channel to an existing agent. Validates credentials, stores the token "
            "in the OS keyring, updates config.json, hot-reloads the router so the new bot is "
            "live immediately. Channels: telegram, discord, slack, imessage.\n"
            "- `list_imessage_chats(limit?)` — lists recent Messages.app chats with their "
            "chat_identifier. macOS only; call before bind_channel(channel='imessage').\n\n"
            "When the user asks you to spin up a new agent, walk them through: (1) create_agent "
            "with their chosen persona, (2) for each channel they want, print the manual setup "
            "steps (BotFather for Telegram, discord.com/developers for Discord, api.slack.com/apps "
            "for Slack, Messages.app for iMessage), (3) wait for the token they paste back, "
            "(4) call bind_channel. Tokens never live in plaintext config — they go to the OS "
            "secret store automatically. WhatsApp is not supported."
        )
